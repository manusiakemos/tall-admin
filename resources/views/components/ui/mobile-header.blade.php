<header x-data="{ isOpen: false }"
    ...
    <div class="flex items-center justify-between">
        ...
        <button @click="isOpen = !isOpen"
                class="text-gray-700 dark:text-white text-3xl focus:outline-none relative">
    ...
    <!-- Dropdown Nav Mobile -->
    <nav x-show="isOpen" class="pt-4 w-full h-full min-h-screen backdrop-filter backdrop-blur-3xl backdrop-sepia-0 bg-white/70 dark:bg-black/70 fixed left-0 top-0 z-50 overflow-scroll" x-transition.duration.500ms>
        <div class="px-6 py-3 flex justify-between">
            <h4 class="heading text-gray-700 dark:text-white">{{config('setting.app_name.value') ?? config('app.name')}}</h4>
            <button @click="isOpen = !isOpen"
